...
public class PageUtil {
	...
	public String getPaging(String path) {
	      ...
	      if(beginPage != 1) {
	         sb.append("<li><a href=\"" + path + "?page=" + (beginPage-1) + "\">◀</a></li>");
	      }
	      
	      // 페이지번호 : 현재 페이지는 링크가 없다
	      for(int p = beginPage; p <= endPage; p++) {
	         if(p == page) {
	            sb.append("<li><a href=\"#\">"+ p + "</a></li>");
	         } else {
	            sb.append("<li><a href=\"" + path + "?page=" + p + "\">" + p + "</a></li>");
	         }
	      }
	      
	      // 다음블록 : 마지막 블록이 아니면 다음블록이 있다
	      if(endPage != totalPage) {
	         sb.append("<li><a href=\"" + path + "?page=" + (endPage+1) + "\">▶</a></li>");
	      }
	      sb.append("</ul>");
